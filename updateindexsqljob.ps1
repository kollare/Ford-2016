...
.DESCRIPTION
    This runbook indexes all of the tables in a given database 
	if the fragmentation is above a defined percentage. 
...
"@
    ...
    # If a table was specified, then find this table if it needs to be indexed, otherwise
    # set the TableNames to $null because we shouldn't process any other tables.
    If ($Table) {
        ...
        Else {
            # Remove other tables since only a single table was specified.
...
"@
        ...
        $Cmd=new-object system.Data.SqlClient.SqlCommand($SQLCommandString, $Conn)
        # Set timeout to be less than 30 minutes since the job will get queued if > 30
...
"@              
                ...
                $Cmd=new-object system.Data.SqlClient.SqlCommand($SQLCommandString, $Conn)
                # Set the Timeout to be less than 30 minutes; the job will get queued if > 30
                # Setting to 25 minutes to be safe.
                $Cmd.CommandTimeout=1500
                # Execute the SQL command
                $Ds=New-Object system.Data.DataSet
            ...
            Else {
                # Catch the exception here so other tables can be processed.
